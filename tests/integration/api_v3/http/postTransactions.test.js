...
describe('Post transactions API', () => {
	...
	it('should post transaction successfully', async () => {
		if (isDevnet) {
			...
			const postTransaction = await api.post(endpoint, { transaction: encodedTx });
	...
	it('should return bad request when posting invalid binary transaction', async () => {
		if (isDevnet) {
			const postTransaction = await api.post(endpoint, { transaction: INVALID_TRANSACTION }, 400);
	...
	it('should return bad request in case of invalid query params', async () => {
		if (isDevnet) {
			const postTransaction = await api.post(endpoint, { transactions: INVALID_TRANSACTION }, 400);
