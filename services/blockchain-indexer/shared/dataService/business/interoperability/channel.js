...
const resolveChannelInfo = async inputChainID => {
	if (inputChainID === (await getCurrentChainID())) {
		throw new ValidationException(
			'Channel info cannot be determined when receivingChainID and currentChainID are same.',
		);
	}
	...
	try {
		...
		const { data: response } = await HTTP.post(invokeEndpoint, {
		...
		if (response.error) {
			throw new ValidationException(
				`Channel info is not available for the chain: ${inputChainID}.`,
			);
		}
		...
		throw new ValidationException(
			`Error while retrieving channel info for the chain: ${inputChainID}.\nError: ${error}`,
		);
	}
};
