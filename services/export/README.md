...
| `SERVICE_EXPORT_PARTIALS` | Local filepath where the partial (per day) transaction history files for a specified account are temporarily cached. |
| `SERVICE_EXPORT_STATIC` | Local filepath where the aggregated transaction history files, as requested by the user are stored. |
...
| `EXPORT_S3_REGION` | The region where Amazon S3 bucket is hosted. Optional. |
| `EXPORT_S3_BUCKET_NAME` | Amazon S3 bucket name. Optional. |
| `EXPORT_S3_BUCKET_NAME_PARTIALS` | Amazon S3 bucket where the partial (per day) transaction history files for a specified account are temporarily cached. |
| `EXPORT_S3_BUCKET_NAME_EXPORTS` | Amazon S3 bucket where the aggregated transaction history files, as requested by the user are stored. |
| `JOB_INTERVAL_CACHE_PURGE` | Job run interval to clean up cache. |
| `JOB_SCHEDULE_CACHE_PURGE` | Job run cron schedule to clean up the cache. |

> **Note**: `interval` takes priority over `schedule` and must be greater than 0 to be valid for all the moleculer job configurations.
> **Note**: The `*EXPORT_S3_**` variables should only be configured when using AWS S3 instead of a local file system. For the local file system, use `*SERVICE_EXPORT_PARTIALS*` and `*SERVICE_EXPORT_STATIC*` variables.
