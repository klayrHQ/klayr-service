...
:url_config_file: https://github.com/KlayrHQ/klayr-service/blob/release/0.8.0-alpha.1/ecosystem.config.js
:url_klayr_service_repo: https://github.com/KlayrHQ/klayr-service
...
On the xref:{url_intro_services}[Introduction to Klayr Service] page, we have discussed the various xref:{url_microservices}[microservices] that combine to form Klayr Service.
On this page, we will thoroughly discuss the configuration options available for each service.

== Default configurations
By default, a Klayr Service installation will have an `ecosystem.config.js` file with all the default configurations that could be used to run and manage the various microservices with {url_pm2}[pm2^].

NOTE: The actual {url_config_file}[`*ecosystem.config.js*`^] file for Klayr Service can be found in the {url_klayr_service_repo}[klayr-service^] GitHub repository.
...
|redis://klayr:password@127.0.0.1:6379/0
...
| redis://klayr:password@127.0.0.1:6379/5
...
| `GATEWAY_ROUTES_CALL_TIMEOUT`
| number
| Timeout (in seconds) for API requests proxied from the gateway to other microservices.
...
Enabling this might be helpful when deploying Klayr Service behind a load balancer or a reverse proxy.
...
| `REQUEST_CACHING_TTL`
| string or number
| Time-to-live for RPC cache entries.
Can be set to `'blockTime'` to cache until the next new block, or a number in seconds.
By default, it is set to `'blockTime'`.
| blockTime

| `REQUEST_CACHING_EXCLUDE_LIST`
| string
| A comma-separated list of RPC methods to exclude from caching.
| `post.transactions,post.transactions.dryrun`
...
| `SERVICE_CONNECTOR_CACHE_REDIS`
| string
| URL of the cache storage (Redis).
| redis://klayr:password@127.0.0.1:6379/10

| `KLAYR_APP_URLS`
| string
| Comma-separated list of Klayr application node URLs to connect to.
| ws://127.0.0.1:7887

| `KLAYR_EVENT_SUBSCRIBER_NODE`
| string
| The Klayr application node URL to subscribe to events from.
| ws://127.0.0.1:7887

| `SERVICE_CONNECTOR_MYSQL`
| string
| Connection string for the MySQL instance that the microservice connects to.
| mysql://klayr:password@127.0.0.1:3306/klayr

| `GENESIS_BLOCK_URL`
| string
| URL of the Klayr SDK-based application's genesis block.
Only to be used when the genesis block is large enough to be transmitted over API calls within the timeout.
| https://downloads.klayr.xyz/klayr/mainnet/genesis_block.json.tar.gz[Genesis block URL^]

| `CLIENT_CONNECTION_VERIFY_INTERVAL`
| number
| Every n milliseconds, verify if client connection is alive.
By default, it is set to `30000` ms.
| 30000

| `CLIENT_POOL_SIZE`
| number
| Number of active API clients to be maintained in the pool for each Klayr application node URL.
...
| `INVOKE_ENDPOINT_QUEUE_CONCURRENCY`
| number
| The number of concurrent jobs to process from the invoke endpoint queue. More concurrent invoke means more RAM consumption.
By default, it is set to `3`.
| 3

| `INVOKE_ENDPOINT_COALESCING_CACHE_TTL`
| number
| The time-to-live (in milliseconds) for the coalescing cache for invoke endpoints.
By default, it is set to `1000`.
| 1000

| `ENABLE_TESTING_MODE`
| boolean
| Boolean flag to enable testing mode.
| false
  ...
  apps: [
    // Configuration for other microservices.
    {
		...
		env: {
			...
			// INVOKE_ENDPOINT_QUEUE_CONCURRENCY: 3,
			// INVOKE_ENDPOINT_COALESCING_CACHE_TTL: 1000,
			// CLIENT_CONNECTION_VERIFY_INTERVAL: 30000,
...
| redis://klayr:password@127.0.0.1:6379/1
...
| `INDEX_MISSING_BLOCKS_SKIP_THRESHOLD`
| number
| Skip threshold for indexing missing blocks.
By default, it is set to `1000`.
| 1000

| `INDEX_MISSING_BLOCKS_MAX_SCHEDULE`
| number
| Maximum number of blocks to schedule for indexing when missing blocks are detected.
By default, it is set to `20000`.
| 20000

| `SCHEDULE_BLOCK_INDEXING_MAX_BATCH_SIZE`
| number
| Maximum batch size for scheduling block indexing.
By default, it is set to `5000`.
| 5000

| `GET_MISSING_BLOCKS_LIST_QUERY_SIZE`
| number
| Maximum query size for getting missing block per one batch.
By default, it is set to `2000`.
| 2000

| `REQUEST_TIMEOUT_RETRY_DELAY`
| number
  ...
  apps: [
    // Configuration for other microservices.
    {
		...
		env: {
			...
			// INDEX_MISSING_BLOCKS_MAX_SCHEDULE: 20000,
			// SCHEDULE_BLOCK_INDEXING_MAX_BATCH_SIZE: 5000,
			// GET_MISSING_BLOCKS_LIST_QUERY_SIZE: 2000,
			// REQUEST_TIMEOUT_RETRY_DELAY: 1000,
...
| string
| URL of the cache storage (Redis).
| redis://klayr:password@127.0.0.1:6379/2

| `SERVICE_INDEXER_REDIS_VOLATILE`
| string
| URL of the volatile cache storage (Redis).
| redis://klayr:password@127.0.0.1:6379/3
...
| `ENABLE_INDEXING_BENCHMARK`
| boolean
| Boolean flag to enable indexing benchmark.
| false

| `KLAYR_STATIC`
| string
| URL of Klayr static assets.
| https://static-data.klayr.xyz
...
| `GET_GENERATORS_LIMIT`
| number
| The number of generators that each block will retrieve to update on generators.change.
By default, it is set to `53`.
| 53

| `INDEX_BLOCKS_RETRY_DELAY`
| number
| The delay in milliseconds before retrying to index a block to reduce node stress.
By default, it is set to `1000`.
| 1000
...
| `DEVNET_MAINCHAIN_URL`
| string
| Devnet mainchain service URL for custom deployments.
| http://devnet-service.klayrdev.net:9901
...
|https://snapshots.klayr.io/mainnet/service.sql.gz
...
| https://github.com/KlayrHQ/app-registry
...
| `DEFAULT_APPS`
| string
| Comma separated values of user default apps other than klayr_mainchain.
| hello_sidechain
|===
...
| string
| URL of the cache storage (Redis).
| redis://klayr:password@127.0.0.1:6379/4
...
| string
| URL of the cache storage (Redis).
| redis://klayr:password@127.0.0.1:6379/7
...
| mysql://reader:password@127.0.0.1:3307/klayr
...
| string
| URL of the cache storage (Redis).
| redis://klayr:password@127.0.0.1:6379/6
...
The free plan would suffice for Klayr Service.
|
...
| `JOB_INTERVAL_REFRESH_PRICES_BITRUE`
| string
| Job run interval to refresh prices from Bittrue.
By default, it is set to `0`.
| 0

| `JOB_SCHEDULE_REFRESH_PRICES_BITRUE`
| string
| Job run cron schedule to refresh prices from Bittrue.
By default, it is set to run `every` minute.
|* * * * *
...
| `JOB_INTERVAL_REFRESH_PRICES_PROBIT`
| string
| Job run interval to refresh prices from ProBit.
By default, it is set to `0`.
| 0

| `JOB_SCHEDULE_REFRESH_PRICES_PROBIT`
| string
| Job run cron schedule to refresh prices from ProBit.
By default, it is set to run `every` minute.
|* * * * *
  ...
  apps: [
    // Configuration for other microservices.
    {
		...
		env: {
			...
			// JOB_SCHEDULE_REFRESH_PRICES_BINANCE: '* * * * *',
			// JOB_INTERVAL_REFRESH_PRICES_BITRUE: 0,
			// JOB_SCHEDULE_REFRESH_PRICES_BITRUE: '* * * * *',
			// JOB_INTERVAL_REFRESH_PRICES_PROBIT: 0,
			// JOB_SCHEDULE_REFRESH_PRICES_PROBIT: '* * * * *',
...
| redis://klayr:password@127.0.0.1:6379/8

| `SERVICE_EXPORT_REDIS_VOLATILE`
| string
| URL of the volatile cache storage (Redis).
| redis://klayr:password@127.0.0.1:6379/9
